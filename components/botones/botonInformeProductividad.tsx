...
export default function BotonInformeProductividad({
    ...
    const getLineaName = (id?: number) => {
        ...
        switch(id) {
            case 15: return t('linea1', "Línea 1");
            case 16: return t('linea2', "Línea 2");
            default: return `${t('linea', "Línea")} ${id}`;
        }
    };

    const getEquipoName = (id?: number) => {
        if (!id || id === 30) return t('todosLosEquipos', "Todos los equipos"); 
        return id <= 6 ? `${t('cocina', "Cocina")} ${id}` : `${t('enfriador', "Enfriador")} ${id-6}`;
    };

    const handleInformeUnificadoDownload = async () => {
        try {
            const productivityContainer = document.querySelector('.productividad-container') as HTMLElement;
            if (!productivityContainer) {
                throw new Error(t('error.productividadContainerNoEncontrado', 'No se encontró el contenedor de productividad para la captura.'));
            }
            ...
            const formattedStartDate = startDate ? startDate.split('T')[0] : sevenDaysAgoFormatted;
            ...
            if (!response.ok) {
                ...
                throw new Error(t('error.apiProductividad', `Error al obtener datos de productividad: ${response.status} - ${detailedError}`, { status: response.status, error: detailedError }));
            }
            ...
            pdf.text(t('pdf.tituloProductividad', "Informe de Productividad"), margin, 8);
            const currentDate = new Date().toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' });
            pdf.text(`${t('pdf.fechaExportacion', "Fecha de exportación")}: ${currentDate}`, margin, 14);
            ...
            const drawDataPageHeader = (isContinuation = false) => {
                ...
                pdf.setFontSize(10);
                pdf.setTextColor(255, 255, 255);
                const pageTitle = isContinuation ? t('pdf.datosProductividadCont', "Datos de Productividad (Cont.)") : t('pdf.datosProductividad', "Datos de Productividad");
                pdf.text(pageTitle, margin, 8);
                pdf.text(`${t('pdf.periodo', "Periodo")}: ${formattedStartDate} - ${formattedEndDate}`, margin, 14);
            ...
            pdf.text(t('pdf.datosGenerales', "Resumen General de Productividad:"), margin, currentY);
            currentY += 6;
            ...
            if (apiData.productos_realizados && apiData.productos_realizados.length > 0) {
                pdf.setFont('helvetica', 'bold');
                pdf.setFontSize(10);
                pdf.setTextColor(0,0,0);
                pdf.text(t('pdf.productosRealizados', "Detalle de Productos Realizados:"), margin, currentY);
                currentY += 7;
                ...
                const maxContentHeightOnPage = pageHeight - pageBottomMargin - metadataHeight;
                ...
                pdf.text(t('pdf.noProductosRealizados', "No hay datos de productos realizados para el periodo seleccionado."), margin, currentY);
            }
            ...
            toast.success(t('exitoTitulo', 'Éxito'), {
                description: t('exito.informeProductividadDescargado', 'Informe de productividad descargado correctamente.'),
                position: 'bottom-right'
            });

        } catch (error) {
            console.error(t('errorGenerarInformeConsola', 'Error al generar informe de productividad:'), error);
            const errorMessage = error instanceof Error ? error.message : t('errorGenerarInformeDesconocido', 'Ocurrió un error desconocido al generar el informe.');
